...
def cg_solve(
    ...
    def Omega_plus_func(x):
        ...
        for k in range(config["n_time"]):
            ...
            # Run Fx through spatial model without bias to obtain S_k @ (F_k @ x_k)
            graph_dummy.x = F_x[k].reshape(-1, 1) # Reshape F_x[k] to [n_space, 1]
            dgmrf_k = dgmrf_list[k]
            S_F_x_k = dgmrf_k(graph_dummy, with_bias=False)

            # Run S_F_x_k through spatial model without bias and transpose equal True to obtain Q_k @ F_k @ x_k = S_k^T (S_k @ F_k @ x_k)
            graph_dummy.x = S_F_x_k
            Q_F_x_k = dgmrf_k(graph_dummy, transpose=True, with_bias=False)
...
def posterior_inference(
    ...
    graph_dummy.x = b_f_k.reshape(-1, 1) 

    # Feed b_f_k through spatial model to obtain model bias b_theta
    for k in range(config["n_time"]):

        # b_theta_k = S_k @ b_f_k + b_s_k
        dgmrf_k = dgmrf_list[k]
        b_theta_k = dgmrf_k(graph_dummy)

        # (1) S_k @ c_k = -b_theta_k, cf. equation (10)
        S_c_k = -b_theta_k

        # (2) Q_k @ c_k = S_k^T @ S_k @ c_k
        graph_dummy.x = S_c_k
        Q_c_k = dgmrf_k(graph_dummy, transpose=True, with_bias=False) 
    ...
    # Reshape Q_c to math the input shape of the temporal model
    Q_c = Q_c.reshape(config["n_time"], 1, config["n_space"]) # here we have n_sample = 1. This dimension is necessary for the temporal model below

    # F^T x = y, which is a vector with elements y_k = x_k - F^T_k x_{k+1} for k = 0, ..., n-2 and y_n = x_n  
    x_k = Q_c[:-1] # [:-1] exclude last time step
    F_T_k_x_k_plus_1 = temporal_model(Q_c[1:].unsqueeze(-1), transpose=True, with_bias=False) # [1:] exclude first time step
    x_n_minus_1 = Q_c[-1].unsqueeze(0) # [-1] last time step
    F_T_Q_c = torch.cat((x_k - F_T_k_x_k_plus_1.squeeze(-1), x_n_minus_1)) 
    ...
    cg_start_guess = cg_start_guess.unsqueeze(0).unsqueeze(2)  # batch dimension - necessary for cg_batch function
